...
func Compose(view memory.View, task string, budget Budget) PromptContext {
	...
	if budget.MaxSummaryChars > 0 && len(summary) > budget.MaxSummaryChars {
		summary = summary[:budget.MaxSummaryChars]
		truncated = true
	}
